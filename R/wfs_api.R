...
#'
#' @examples
#'   wfs_api(base_url = "https://kartta.hel.fi/ws/geoserver/avoindata/wfs", 
#'           queries = append(list("service" = "WFS", "version" = "1.0.0"), 
#'                 list(request = "getFeature", 
#'                      layer = "tilastointialueet:kunta4500k_2017")))
#'
wfs_api <- function(base_url = "https://kartta.hel.fi/ws/geoserver/avoindata/wfs", queries) {
  
  if (!grepl("^http", base_url)) stop("Invalid base URL")
  
  # Set the user agent
  ua <- httr::user_agent("https://github.com/pitkant/cityhoods")
